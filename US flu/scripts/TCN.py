...
def main(horizon, mode, model_name, seed, params,train_end_date,val_end_date,test_end_date,year):
    ...
    train_data1 = data_util.get_data('2003-02-02', '2008-10-26')
    ...
    tcn_old = train_tcn(tcn_model, train_data, val_data, num_epochs=params['epoch'], batch_size=params['batch_size'], lr=params['lr'],weight_decay=params['weight_decay'], plot=False)  # 0.0007
    ...
    for i in range(90):
        ...
        if '_ol' in mode:
            tcn_new = train_tcn(copy.deepcopy(tcn_old), new_train_data, new_train_data, num_epochs=2, batch_size=4,
                        lr=params['lr'] / 10, plot=False, froze=True)  # epoch=1,lr=0.0001
        else:
            tcn_new = tcn_old
        ...
        #         data, tcn_new, test_data, train_scaler, next_date, pm_mode,
        ...
        # prediction
        df_true, df_pred = create_prediction_dataframe(
            data, tcn_new, new_train_data, train_scaler, next_date, 'train',
    ...
    # format_pred_df.to_csv('../res/tcn_base_stage_holi_pf.csv', index=False)
