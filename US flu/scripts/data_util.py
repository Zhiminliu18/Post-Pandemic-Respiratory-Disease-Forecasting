...
def get_data(scale_k = 1.0,path2='../res/particle_filter_results_usa.pkl'):
    data = pd.read_csv('../data/flu_data_7.csv')
    with open(path2, 'rb') as f:  # 注意是 'rb'（二进制读取模式）
...
def calc_quantiles(row, k=220, quantiles = QUANTILES ):
    t = row['week_ahead']
    k = k-15*(t+1)
    if row['point']<=0:
        row['point']=1e-3
