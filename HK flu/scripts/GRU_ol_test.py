...
def main(data,permutation,horizon, mode, model_name, seed, params,train_end_date,val_end_date,test_end_date,year):
    ...
    train_data3 = data_util.get_data('2023-03-19', train_end_date,now_forecast=True)  #用截至到end_date的数据，用mask遮盖未来数据，防止信息leak
...
if __name__ == "__main__":
    data = get_data()
    # data.to_csv('../res/flu_data_all_hk.csv', index=False)
    horizon = 9
    model_name = 'gru'
    permutation = False
    modes = ['base','base_stage_holi_pf']
    # modes = ['base_stage','base']
    seeds = [42] #123,456
    params1 = {
        'lr' : 0.0003,
        'batch_size' : 16,
    ...
    for mode in modes:
        for seed in seeds:
            main(data,permutation,horizon, mode, model_name,seed, params1,train_end_date = '2023-08-06', val_end_date = '2023-11-26',
                 test_end_date = '2024-12-01',year=2024)
            main(data,permutation,horizon, mode, model_name, seed, params2, train_end_date='2023-11-26', val_end_date='2024-09-15',
