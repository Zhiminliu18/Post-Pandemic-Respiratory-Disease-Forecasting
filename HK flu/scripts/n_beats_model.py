...
import torch
torch.set_printoptions(precision=8, sci_mode=False)  # 设置8位小数，非科学计数法
...
def train(model, train_data, val_data, num_epochs=10, batch_size=16, lr=0.001, weight_decay=10e-8,save_path='best_model.pth',plot=True,froze=False):
    train_losses = []
    val_losses = []
    ...
    for epoch in progress_bar:
        ...
        for x_batch, holiday, y_batch, mask in train_loader:
            optimizer.zero_grad()
            output_ili, output_gr,output_bt,output_I_gr = model(x_batch, holiday)
        ...
        with torch.no_grad():
            for x_batch, holiday, y_batch, mask in val_loader:
                output,output_gr,output_bt, output_I_gr = model(x_batch, holiday)
...
def train_itrans(model, train_data, val_data, num_epochs=50, batch_size=16, lr=0.0012, weight_decay=1e-8, save_path='best_model.pth',plot=True,froze = False):
    train_losses = []
    val_losses = []
    best_model = copy.deepcopy(model)
    train_data = TensorDataset(
...
def train_tcn(model, train_data, val_data, num_epochs=10, batch_size=16, lr=0.001,weight_decay=1e-8, save_path='best_model.pth',plot=True,froze=False):
    train_losses = []
    val_losses = []
    ...
    optimizer = optim.AdamW(model.parameters(), lr=lr, weight_decay=weight_decay)#, tcn2024 weight_decay=1e-8 nbeats:weight_decay=1e-8
    mae_loss = nn.L1Loss(reduction='mean')
    best_val_loss = float('inf')  # 用于跟踪最佳验证损失
    if froze:
        for name, param in model.named_parameters():
            if 'hist_encoder' in name.lower():
                param.requires_grad = False
                # print(f"冻结层: {name}")
    progress_bar = tqdm(range(num_epochs), desc="Training Progress", dynamic_ncols=True, leave=True)
    for epoch in progress_bar:
        ...
        for x_batch, holiday, y_batch, mask in train_loader:
            ...
            output_ili = model(window_batch)
            huber_loss_value = weighted_mae(output_ili[:, :, 0], y_batch[:, :, 0],mask[:, :, 0])
            total_loss = huber_loss_value #+ smoothness_penalty(output_ili[:, :, 0])
            total_loss.backward()
            optimizer.step()
            train_loss += total_loss.item()
...
def get_val_predictions(model, val_data, scaler, if_res=False,shuffle_feature=False, feature_idx=None,timestep_idx=None,seed=42):
    ...
    if shuffle_feature:
        ...
        if timestep_idx is None:
            ...
            target_data = X_val[:, timestep_idx, feature_idx].flatten()  # 当前时间步的所有样本的特征值
            shuffled_data = np.random.permutation(target_data)  # 打乱当前时间步的数据
            X_val[:, timestep_idx, feature_idx] = shuffled_data  # 覆盖原数据

        if feature_idx==5:
            X_val = replace_zero_with_one(X_val, feature_idx, replace_ratio=0.2)
    ...
    with torch.no_grad():
        for x_batch, holiday, y_batch,_ in val_loader:
            ...
            preds_scaled, preds_gr ,output_bt, output_I_gr= model(x_batch,holiday)
...
def get_tcn_val_predictions(model, val_data, scaler, if_res=False,shuffle_feature=False, feature_idx=None,timestep_idx=None,seed=42):
    model.eval()
    scaler_target = scaler['y']
    X_val = val_data['X'].copy()
    if shuffle_feature:
