...
def get_data(scale_k = 1.0,data_path = '../data/flu_data_final.csv', pf_path='../res/particle_filter_results_103.pkl'):
    data = pd.read_csv(data_path)
    data['date'] = pd.to_datetime(data['date'])
    data['month'] = data['date'].dt.month

    def assign_season(month):
        if month in [12, 1, 2, 3, 4, 11]:
            return 1  # 冬春季
    ...
    #
    # school_absent_data = pd.read_csv('../data/school_absent_data.csv')
    # school_absent_data['date'] = pd.to_datetime(school_absent_data['date'])
    # data = pd.merge(
    #     data,
    #     school_absent_data,
    #     on='date',
    #     how='left',
    # )
    # mean_value = data['Total_absent_rate'].mean()
    # data.loc[data['date'] < '2008-09-18', 'Total_absent_rate'] = -1
    # data['Total_absent_rate'] = data['Total_absent_rate'].fillna(0)
    ...
    data['sin_week'] = np.sin(2 * np.pi * data['flu_weekid'] / 52)
    data['cos_week'] = np.cos(2 * np.pi * data['flu_weekid'] / 52)
    ...
    def add_exponential_decay_distance(group):
        ...
        return group
    data = data.groupby('flu_season_year').apply(add_exponential_decay_distance)
    return data
...
class DataPrepare:
    ...
    def get_data(self, start_date, end_date,threshold=1, now_forecast = False):
        ...
        # mask  = np.ones((len(valid_indices),self.horizon,1))
        # if now_forecast:
        #     for i in range(self.horizon):
        #         pos = self.horizon-i
        #         last_pos = i+1
