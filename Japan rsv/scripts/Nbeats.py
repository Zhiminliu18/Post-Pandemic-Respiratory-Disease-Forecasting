...
def main(horizon, mode, model_name, seed,  params,train_end_date,val_end_date,test_end_date,year):
    ...
    NBEATSx_model = NBEATSx(h=horizon, input_size=seq_l,
                    ...
                    n_blocks = [2],     # 2,108
                    mlp_units = [[108,108],[108,108]]
                    )


    train_data2 = data_util.get_data('2013-04-07', '2019-12-15')
    ...
    Nbeats_model_old = train_tcn(NBEATSx_model, train_data, val_data, num_epochs=params['epoch'], batch_size=params['batch_size'], lr=params['lr'],weight_decay=params['weight_decay'],plot=False) #0.0003
    ...
    for i in range(90):
        ...
        if '_ol' in mode:
            Nbeats_model_new = train_tcn(copy.deepcopy(Nbeats_model_old), new_train_data, new_train_data, num_epochs=2, batch_size=4, lr=params['lr']/5,plot=False,froze=False) # epoch=1,lr=0.0001
        else:
            Nbeats_model_new = Nbeats_model_old

        # for i in range(len(data_util.features)):
        #     pm_mode = f'feature{i}'
        #     df_true, df_pred = create_prediction_dataframe(
        #         Nbeats_model_new, test_data, train_scaler, next_date, pm_mode,
        ...
        df_true, df_pred = create_prediction_dataframe(
            Nbeats_model_new, new_train_data, train_scaler, next_date, 'train',
